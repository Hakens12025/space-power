...
**大序列 = 舰级的"用哪几条序列"**(`s.fcBig`:`'rr'` 轮询,默认 / `'pick'` 选择),与序列内的 `q.mode`(`'seq'` 依次 / `'rr'` 轮询,管"这条序列里先打谁")是**两个层级**。两级都有一个叫"轮询"的选项,所以按钮标签恒写作「大序列:轮询 / 大序列:选择」,**作用域前缀写死在文案里** —— 同一手法见轮盘左半环的「整条序列 / 仅此目标」。选择模式下 `s.fcPick` 那一条独占火力,序列即火力模板。
...
**暂停不再用 `opacity` 变灰**:在这个本来就偏暗的面板上几乎看不出来。改红色实态,且**刻意不叠 opacity** —— 压暗会把红一起压掉,结果既不红也不明显。详情区同步给一条红标,否则方条红了而展开的详情没提示,读着断裂。
