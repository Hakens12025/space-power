...
**落地方式:只藏不删**(同 RF2/RF6 惯例)。`engMode='tri'` 为默认;顶栏 `#engSw` 切换钮加进 css 隐藏清单(复活 = 删那一行);`classic`/`torque` 的代码分支原样保留。**由此新失效的代码**(有 UI 才可达,现全部不可达):`steerToVel` 的 classic 阈值功率块、`stepAttitude` 主体与 `applyHeading` 的 torque 分支、`ENG_ALPHA/ENG_OMEGA_CAP/ENG_KP/ENG_KD/ENG_DEAD/EPOD_ARM`、`engNozzles` 的差模参数 `h`(现恒传 0)、`ENG_MODES/ENG_LABEL` 与 85-settings 的 `engSw` 委托 —— 按全局规矩列出待用户拍板后才删。
