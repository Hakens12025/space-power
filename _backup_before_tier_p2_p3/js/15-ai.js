...
function openCtx(sx,sy,onShip){
  ...
  if(onShip&&onShip.side==='red'&&!adminMode){ // 普通模式敌舰:交战菜单;GM下右键敌舰走控制菜单
    ...
    const hasMac=attackers.some(s=>s.cls==='CRUISER'||s.cls==='FRIGATE');
    ...
    const items2=[
      ...
      {t:'🎯 机动对准·MAC齐射',enabled:hasMac,run:()=>{ // DS171:M3 lockPlayer→driftFire(命令照走找窗口,对准瞬间齐射)
        attackers.forEach(s=>{if(s.cls==='CRUISER'||s.cls==='FRIGATE'){s.lockedTarget=onShip;s.driftFire=true;s.driftFireT=60;}});
      ...
      {t:'💥 全弹发射(选中舰)',enabled:hasMac||hasMis,run:()=>{let n=0;attackers.forEach(s=>{if((s.cls==='CRUISER'||s.cls==='FRIGATE')&&s.macCd<=0&&macAligned(s,onShip)){fireMAC(s,onShip);n++;}if(s.ammo>0){orderMissileSalvo(s,onShip,salvoCount);n++;}});log(n?`${n} 次全弹发射`:'武器未就绪','');}},
    ];
