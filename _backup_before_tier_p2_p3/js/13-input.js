    ...
    if(t&&!t.dead){ // 点中舰船:按攻击方各自阵营探测门控(GM能指挥敌方,但各边只能打自己探测到的)
      {
        ...
        if(hiters.length){
          if(selWeapon==='mac'){hiters.forEach(x=>{if(x.cls==='CRUISER'||x.cls==='FRIGATE'){x.lockedTarget=t;x.driftFire=true;x.driftFireT=60;}});log(`🎯 ${hiters.length} 艘 MAC攻击 ${t.name} · 漂移射击60s,对准即发`,'');} // DS171:M3 lockPlayer→driftFire
