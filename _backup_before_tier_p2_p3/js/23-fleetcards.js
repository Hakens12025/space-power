...
function renderFleet(){
  ...
  ships.forEach(s=>{
    ...
    c.appendChild(dot);c.appendChild(nm);c.appendChild(st);
...
function updateInfo(){
  ...
  if(sel.length===1){
    ...
    infoEl.innerHTML=`<h3>${s.name}</h3>
      <div class="row"><b>舰种</b><span>${CLS_NAME[s.cls]||s.cls}</span></div>
      ...
      <div class="row"><b>速度令</b><span>${s.speedCmd===null?'—':SPEED_NAMES[s.speedCmd]||s.speedCmd}</span></div>
      <div class="row"><b>结构</b><span>${Math.round(s.hp)}/${s.maxHp}</span></div>
      ...
      <div class="row" style="gap:4px">
        ...
        ${s.cls==='SCOUT'?`<button class="mini" data-kind="ship" data-action="beacon">🛰 信标(${s.beaconCount}/2)</button>`:''}
