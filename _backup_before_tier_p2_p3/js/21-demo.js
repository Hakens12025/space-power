...
function snapshot(){
  return {
    ...
    ships:ships.map(s=>({id:s.id,name:s.name,cls:s.cls,side:s.side,pos:s.pos.slice(),vel:s.vel.slice(),facing:s.facing.slice(),hp:Math.round(s.hp),spd:s.speedCmd,orders:s.orders.map(o=>({pos:o.pos.slice(),t:o.type})),fm:s.formation?{dest:s.formation.dest.slice(),slot:(s.fmSlot||[0,0,0]).slice()}:null,lock:s.lockedTarget&&!s.lockedTarget.dead?s.lockedTarget.name:null,lit:s.side==='red'?s.litBlue:s.litRed})),
