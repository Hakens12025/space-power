...
function enemyAI(dt){ // 叛军AI:朝【它认为】玩家在的地方推进/锁定/开火/被MAC锁定时规避;感知v4:只打红网络点亮的蓝舰
  ...
  for(const e of ships){
    ...
    if(nearest&&e.macCd<=0&&hasMAC(e)&&d<((typeof macEffRange==='function')?macEffRange(e):(e.macRange||150000))&&macAligned(e,nearest))fireMAC(e,nearest); // 敌MAC 近距精确;RF3 射程读烘焙字段;TIER1 MAC 舰种门改能力谓词
    ...
    const mirror=(typeof matchIsOn==='function'&&matchIsOn()),rdy=readyCells(e);
    if(nearest&&e.ammo>0&&d<(e.mslRange||350000)&&Math.random()<0.08&&(!mirror||rdy>=Math.ceil((e.cells||4)/2)))orderMissileSalvo(e,nearest,mirror?Math.min(2,rdy):(e.cells||4)); // DS167(设计师拍板):2%→8%,对标bot节奏;RF3 射程读烘焙字段
