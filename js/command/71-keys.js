...
function loadBindings(){try{const r=localStorage.getItem('sp_keys_v1');if(r){bindings=JSON.parse(r);}}catch(e){} // 2026-09-22 起只读不写:键位重绑 UI 随设置面板删除,旧存档里已删动作的键无害(匹配循环走 ACTIONS 不走 bindings)
...
function doAction(id){
  switch(id){
    case 'pause':running=!running;break;
    ...
    case 'turn_cmd':{ // V:船头转向命令——点地图设定方向(调头,速度不变);再按V取消。FM3-0:Shift+V"单纯转头"分支删除(pendingTurnNoFm 只喂船上一个写-only 的死标志)
      if(pendingTurn){pendingTurn=null;if(typeof updSelWeaponTip==='function')updSelWeaponTip();break;} // FL1:同上,清 pendingTurn 必须同步刷 #cmdTip
      const sel=controlledShips(); // FM2:选中什么就转什么(原 expandToFleet 会把单选一艘扩成整组)
      if(sel.length){
        /* FL1 三个 arm 点(底栏跟随 / toggleWeapon / turn_cmd)写法一致:武装前先 clearPendings()。
...
window.addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape'&&typeof rad!=='undefined'&&rad.open){ // RF5 Phase C 轮盘抢 Esc:必须早于下面的 ACTIONS 匹配。放在最前是「任何模式下 Esc 先关轮盘」的一行保险(零耦合;正常情况下 radTick 在测距里已自关)
    e.preventDefault();
    if(typeof radClose==='function')radClose();
    return;
  }
  if(e.key==='Escape'&&typeof tutIsOpen==='function'&&tutIsOpen()){ // RF5-D 教程接 Esc:排在轮盘【之后】——轮盘开着 Esc 先关轮盘(RF5 Phase C 那条语义一行未动),它关掉了才轮到教程。同样必须早于下面的 ACTIONS 匹配
    e.preventDefault();
    tutToggle(false);
    return;
  }
...
window.addEventListener('keyup',e=>{
  ...
  if(e.code==='ControlLeft'||e.code==='ControlRight'){ // Ctrl臂:松开时若未组合其他键 → 全弹发射
    ...
    // 要恢复:把上面的 FIRE_ALL_ON 改回 true。(原来还有一条"日志计数在门控之前自增会骗人",2026-09-22 日志系统整体删除后不再成立。)
    ctrlArm=false;
  }
});
function camHeld(dt){
