...
const FP_KNOBS = [
  ...
  { k: 'pref', nm: '要害偏好', tip: '0 = 所有站位一视同仁,谁去哪只看契合度;越大越把好舰往要紧的站位上塞（"要紧"= 该站位要的那一维的实测影响力,通道最重、隐蔽最轻,再乘本站位表的偏向表）。\n\n【看哪个数】拉高它,上面的「总契合」会略降 —— 那是不加权的和,而算法最大化的是加权和,任何偏向都必然压低它。要看的是它旁边的「要害契合」:实测 13 舰异构编队 pref 0→0.5,总契合 10.56→10.35(−2%),而要害契合 0.50→1.00,代价落在最不要紧的站位上。' },
