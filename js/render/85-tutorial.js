...
<article class="tut">
  ...
  <section class="tut-sec" id="tut-command">
    ...
    <p>方条上方那个按钮管的是<strong>大序列</strong>，也就是这艘舰在多条序列之间怎么分配火力，注意它和序列内部的依次／轮询不是一回事，一个管"用哪几条序列"，一个管"这条序列里先打谁"。默认是<code class="ui">大序列:轮询</code>，多条序列轮流开火。切成<code class="ui">大序列:选择</code>之后，只有你选中的那一条在打，其余的排着队不动——序列在这个模式下就成了火力模板：把几套打法分别排好，需要哪套点哪套。被选中的那条方条上会有一个青色的星号；想换一套，在选择模式下点另一根方条即可。</p>
