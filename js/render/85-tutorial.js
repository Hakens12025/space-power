...
<article class="tut">
  ...
  <section class="tut-sec" id="tut-command">
    ...
    <p>有两件事容易想岔。第一，指针是逐武器分开的，主炮和导弹各走各的：主炮 <code class="num">30 秒</code>一发、导弹一组装填 <code class="num">60 秒</code>，共用一个指针会互相拖着走。所以同一条序列里，主炮打的目标和导弹打的目标经常不是同一个，这是正常的。第二，一艘舰可以有多条序列，默认情况下武器在序列之间也会轮转：这一次从这条序列里找到目标开了火，下一次就从下一条序列开始找。这个轮转可以关掉，见下面的大序列。</p>

    <p>建好的序列会立刻出现在右栏的火控计算机里：五根竖直方条，一根一个序列槽，最多五条。点一根方条进入它的序列态——方条高亮，地图上亮出这条序列的蓝色数据链（舰连到目标一、目标一连到目标二，像地图上的铁路线），方条下方给出这条序列的简要详情；再点同一根就退出序列态，蓝线熄灭。序列态那条也是长按中键三种上下文所依据的当前编辑序列。详情里每个目标可以单独删掉，整条序列可以暂停下来不参与解算，也可以直接删除。暂停的序列会画成红色虚线框加一道对角斜线，像被划掉一样，方条和详情都是——虚线表示"这条不在生效"，斜线表示"禁止"，因为"这条一发不打"是个需要一眼看见的状态。</p>

    <p>一艘舰有多条序列时，默认它们<strong>轮流开火</strong>：这一次从这条序列里找到目标打了，下一次就从下一条开始找。如果你想让它专用某一套打法，点方条进入那条序列的序列态，然后按「火控计算机」标题右边的<code class="ui">选择</code>按钮——从此只有这一条在打，其余的排着队不动，被选中的那条方条上会多一个青色星号。序列在这个用法下就成了火力模板：把几套打法分别排好，需要哪套点哪套。再按一次<code class="ui">选择</code>就回到轮流开火。注意这个"轮流"和序列内部的依次／轮询不是一回事，一个管"用哪几条序列"，一个管"这条序列里先打谁"。</p>

    <p>三种状态可以同时落在同一根方条上，各占一种颜色：黄色边框是你正在查看的序列态，青色星号是选择模式下唯一在打的那条，红色是已暂停。</p>
  </section>

  <section class="tut-sec" id="tut-cmdbar">
    <h2 class="tut-h2">底栏五个开关</h2>

    <p>顺带说一下右栏「实时状态」里的两行读数。<strong>速度</strong>是当前速度矢量的长度；<strong>加速度</strong>是这一拍<em>实际</em>产生的加速度，不是舰船的额定推力——接近期望速度时推力会自动收小，读数跟着降，画面上尾焰也在收，两者是一致的。数值右侧是四盏常驻的推进器指示灯，亮起表示正在点火、半隐表示待机——同一时刻常有多盏亮着（多舱协同）：<code class="ui">主推</code>是船尾蓝焰，<code class="ui">反推</code>是船头橙焰（刹车用，推力与主推相同），<code class="ui">侧推</code>是横向机动——三个推进器舱按 120° 分解合力，任何方向都有接近满额的推力（横向机动时你会看到主推和侧推同时点亮，那是多舱协同，不是故障），而<code class="ui">姿态</code>指的是纯转向——它同样点着侧向推进器，但只改朝向不改速度矢量，所以那一行的加速度是零。看见"姿态"就知道船在转头而不是在变速。还有一件事值得先知道：减速段的引擎是一下一下脉冲着烧的。速度已经贴着期望值时，点火和熄火之间留着一条死区，速度越高这条死区越宽，所以尾焰会按秒级的节奏点了又熄——那是正常的，不是画面在抖。</p>
  ...
  <section class="tut-sec" id="tut-keys">
    ...
    <table class="tut-table tut-mouse">
      ...
      <tbody>
        ...
        <tr><td><code class="key">Shift</code> + 中键短按</td><td>选定目标入链</td><td>加进当前编辑序列（无序列则新建），地图亮出蓝色数据链；重复点同一目标不会重复入链</td></tr>
