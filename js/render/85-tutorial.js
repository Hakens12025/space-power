...
<article class="tut">
  ...
  <section class="tut-sec" id="tut-command">
    ...
    <p>建好的序列会立刻出现在右栏的火控计算机里：五根竖直方条，一根一个序列槽，最多五条。点一根方条进入它的序列态——方条高亮，地图上亮出这条序列的蓝色数据链（舰连到目标一、目标一连到目标二，像地图上的铁路线），方条下方给出这条序列的简要详情；再点同一根就退出序列态，蓝线熄灭。序列态那条也是长按中键三种上下文所依据的当前编辑序列。详情里每个目标可以单独删掉，整条序列可以暂停下来不参与解算，也可以直接删除。暂停的序列会整条变红，方条和详情都是，因为"这条一发不打"是个需要一眼看见的状态。</p>
