...
<article class="tut">
  ...
  <section class="tut-sec" id="tut-cmdbar">
    ...
    <p>顺带说一下右栏「实时状态」里的两行读数。<strong>速度</strong>是当前速度矢量的长度；<strong>加速度</strong>是这一拍<em>实际</em>产生的加速度，不是舰船的额定推力——接近期望速度时推力会自动收小，读数跟着降，画面上尾焰也在收，两者是一致的。数值下面列着此刻在用的推进器，多个同时在推就逐行列出：<code class="ui">主推</code>是船尾蓝焰，<code class="ui">反推</code>是船头橙焰（刹车用，推力与主推相同），<code class="ui">侧推</code>是横向机动——三个推进器舱按 120° 分解合力，任何方向都有接近满额的推力（横向机动时你会看到主推和侧推同时点亮，那是多舱协同，不是故障），而<code class="ui">姿态</code>指的是纯转向——它同样点着侧向推进器，但只改朝向不改速度矢量，所以那一行的加速度是零。看见"姿态"就知道船在转头而不是在变速。还有一件事值得先知道：减速段的引擎是一下一下脉冲着烧的。速度已经贴着期望值时，点火和熄火之间留着一条死区，速度越高这条死区越宽，所以尾焰会按秒级的节奏点了又熄——那是正常的，不是画面在抖。</p>
