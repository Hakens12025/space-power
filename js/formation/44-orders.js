...
/* FM3-0:fmReassign 从 42-formation 原样搬到这里 —— 它是【阵位态下令那一刻】的槽位配对,唯一调用者是下面的 fmSpread,
   放在实体层让 42 成了杂物间。跟随态的持续配对 fmFollowReslot 仍留在 42(由 43-step 每 tick 调)。函数体一行未改。 */
function fmReassign(F, mates, ca, sa, dest, from) {
  /* FL3【每段重新配对槽位】,消除航线交叉。
     ...
     三条约束:
       · 只在【同桶内】换。FM3-2:桶 = 40-slots fmDoctrineSplit 的居中/环上(改前按舰种角色表分主力/护卫/侦察),
  ...
  mates.forEach((m, i) => {
    ...
    /* FM4:分桶键换成【可互换性签名】fmSwapKey(39-fmcaps)——九维能力读数 + inner 全都相同才同桶。
       ...
       FL3 的收益全保留。改前的桶是 fmDoctrineSplit 的居中/环上 + 能力分细分,那是防空环时代的口径:
