...
/* FM6 清理:站距三档预设(贴身/标准/疏开)的 setter 已删 —— 它的三个按钮在 FM5d 随"密度/档位是死钮"
   一并去掉(实测舰数不超过插槽数时站距乘数恒无效),此后零调用点。站距现在只由编组控制页的滑块与切站位写。 */
...
/* FM6:切模式那个 setter 整个删除。它的职责是写【运动方式】这个轴(static / follow),而"编队内部跟随态"
...
/* FM6:跟随态那份「每 tick 连续重配槽位(带迟滞)」整个删除 —— 它只服务于已去掉的【编队跟随模式】。
