...
function cornerSpd(s,vIn,vOut){ // 拐角几何限速
  ...
  /* RF21 曲率限速(用户实报:密集点组成的弧形航线大概率冲过头)。
     本式原来只看【单个拐角】的偏折:弧被离散成密集点后每拐只有 5~14 度,给出的过弯半径大到没有约束
     (11.5 度时约 20 万 km);但这些小角密集连续出现,真实曲率半径是 点距/偏折角 ≈ 弧的半径(1~3 万),
     船在巡航下能跟的最小半径却要 v²/a≈4.7 万 —— 每一步都合法,连起来物理上不可行。
     实测 R=15k~25k 的弧:偏靠全部饱和在 4998~4999(冲出去再绕回来碰点的痕迹),R=20k 用时 234.6s
     而弧长按可跟速度只要约 126s —— 近一倍时间花在冲出/折返上。
     修法:过弯半径取两者较小 —— 单拐角几何半径(原式)与【局部曲率半径】出段弦长/偏折角
     (从圆上采样的折线,弦长/偏折角 恰好还原圆半径)。孤立大角时曲率半径远大于原式(行为逐位不变,
     保住全部已调优结果:90 度孤立拐 9549 vs 原式 2414;之字 117 度 8770 vs 1096),密集小角时它生效
     (R=20k 弧:19930 vs 原式 197900)。
     【只用出段,不用 min(进,出)】:routeCap 里第一个拐角的入段是"船位→拐点"的实时距离,船越逼近它越短,
     min 会把限速人为收紧成"逼近任何拐角都额外刹车"—— 实测把 RF14 切角细化的收益从 7.3% 干到 0。
     曲率约束的口径必须是【折线的静态弦长】:每条弦都是上一个拐角的出段(入段约束已由递推的 reach
     从上游传来),首段是直线进场、本就没有采样曲率可言。 */
  const rTol=c>0?ROUTE_TOL*c/(1-c):0;
  const rCurv=lv/ang;
  return Math.sqrt(Math.max(0,s.thrust*GUIDE_EFF*Math.min(rTol,rCurv)));
}
